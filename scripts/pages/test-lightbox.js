export function lightboxFactory() {
    ...
    function buildLightboxDOM(currentMedia, medias) {
      ...
      if (fileExtension === "mp4") {
        html += `<video><source src="${currentMedia.link}" type="video/mp4"></video>`;
      } else {
        html += `<img src="${currentMedia.link}" alt="">`;
      }
  
      // Ajoute la description du média courant au contenu HTML
      html += `<div class="lightbox-description"><span>${currentMedia.title}</span></div>`;
